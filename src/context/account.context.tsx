...
export const AccountProvider = ({ children }: { children: ReactNode }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (result.success) {
        if (userData.avatar === "/avatars/base.jpg") {
          const resultUpdateAvatar = await updateUserAvatar(userId, result.avatarUrl)

          if (!resultUpdateAvatar.success) {
            setAction({ error: true, message: resultUpdateAvatar.error })
            return
          }
        }
